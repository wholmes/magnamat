/**
 * Client-side cart (localStorage). Wire Stripe Payment Links or Gumroad by setting CHECKOUT_URL.
 * Optional: append ?quantity=N for single-SKU carts (supported by many hosted checkouts).
 */
...
/** Stripe Payment Link, Gumroad permalink, etc. Leave empty until live. */
const CHECKOUT_URL = '';
...
function subtotalParts(lines) {
  ...
  for (const ln of lines) {
    ...
    if (!p || p.priceCents == null) {
      return { primary: 'At checkout', secondary: 'Add priceCents in js/cart.mjs to show a subtotal, or rely on Stripe / Gumroad.' };
    }
...
function render() {
  ...
  if (!lines.length) {
    ...
    if (hint) {
      hint.textContent = sub.secondary || (hasCheckout ? 'You will complete payment on Stripe or Gumroad.' : 'Set CHECKOUT_URL in js/cart.mjs when your product is live.');
    }
  }
